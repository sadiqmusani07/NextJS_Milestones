console.log("Task 2");
var studentName = "saDiq muSani";
console.log("Hello " + studentName + ", would you like to learn some Python today?");
console.log("Task 3");
//This is a comment for tas 10
// Assigning upperCase LowerCase and TitleCase
var upperCase = studentName.toUpperCase();
var lowerCase = studentName.toLowerCase();
var titleCase = "";
...
var person = "  Quaid-e-Azam \nMuhammad Ali \tJinnah  ";
console.log("Person with White Spaces:" + person);
var clean = person.trim().replace(/[\n\t]/g, "");
...
console.log("Task 11");
var friends = ["Suhayb", "Yahya", "Saheeb", "Muzammil"];
...
var modes = ["Car", "Aeroplance", "train"];
modes.forEach(function (item) {
    statements.forEach(function (subItem) {
        console.log(subItem + item);
    });
});
console.log("Task 14");
var invitees = ["Zia Khan", "Kamran Tessori", "Daniyal Nagori"];
...
var middle = Math.floor(invitees.length / 2);
...
if (invitees != null && invitees != undefined) {
    for (var i = 0; invitees.length > 2; i++) {
        var uninvited = invitees.splice(0, 1)[0];
        console.log("Sorry " + uninvited + " we have removed you from the guest list");
    }
}
...
console.log(countries);
var reversedCountries = countries.sort(function (a, b) { return b.localeCompare(a); });
...
console.log("task 22");
var myArr = ["infinix", "Oppo", "Samsung", "Vivo", "Apple"];
//let arr = myArr[10]; //this line throws intentional Error because we have only 5 index array (0 to 4)
var arr = myArr[2];
console.log(arr);
console.log("Task 23");
var car = ["Subaru", "Toyota", "Suzuki", "Lambo", "Buggati"];
console.log("Is Is car.includes('Subaru')? I predict True. && Is car.includes('subaru')? I predict false");
...
console.log("Task 29");
var favorite_fruits = ["Grapes", "Water Melon", "Apple"];
if (favorite_fruits.includes("Grapes")) {
    var msg = "I really like ";
    msg += "Grapes";
    console.log(msg);
}
...
console.log("Task 30");
var users = ["Muhammad", "Sadiq", "Admin", "Ghazali", "Zulfiqar"];
...
current_users = current_users.map(function (user) { return user.toLowerCase(); });
var new_users = ["Muhammad", "Sadiq", "Shafi", "Saad", "Zulfiqar"];
...
ordinalNumber.forEach(function (num) {
    if (num == 1) {
...
console.log("Task 34");
var fav_pizza = ["Pepperoni Pizza", "Malai Boti Pizza", "Chicken Tikka Pizza"];
...
console.log("Task 40");
var make_album = function (artist, albumTitle, nooftracks) {
    if (!nooftracks) {
        var newObject = {
            Artist: artist,
            Title: albumTitle
...
console.log("Task 41");
var magicians = ["David Copperfield", "Derren Brown", "Harry Houdini"];
...
function make_great(magic) {
    return magic.map(function (item) { return "the Great " + item; });
}
...
function Sandwich() {
    ...
    var message = "Sandwhich contains ";
    if (ingredients.length === 0) {
        message += "no items, please provide some item";
    }
    else {
        message += "the below items:\n";
        ingredients.forEach(function (item, index) { return (message += "".concat(index + 1, ".") + item + "\n"); });
    }
    return message;
}
